                    ...
                    >
                        ...
                        >
                            <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 ${isWon ? 'bg-green-100' : 'bg-red-100'}`}>
                                {isWon ? <Star className="w-12 h-12 text-green-600" fill="currentColor" /> : <RotateCcw className="w-12 h-12 text-red-600" />}
                            </div>

                            <h2 className={`text-3xl font-black mb-4 leading-tight ${isWon ? 'text-primary' : 'text-slate-800'}`}>
                                {isWon ? victoryMessage : "Não foi dessa vez!"}
                            </h2>

                            {isLost && (
                                <p className="text-slate-600 font-bold mb-2">
                                    A palavra era: <span className="text-primary text-xl block mt-1 tracking-widest">{wordData.term}</span>
                                </p>
                            )}

                            <p className="text-slate-600 font-bold mb-8">
                                {isWon ? `Você salvou todas as flores do tema ${category.name.toLowerCase()}!` : "As flores caíram, mas você pode tentar novamente!"}
                            </p>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                >
                                    {isWon ? "Jogar de Novo" : "Tentar de Novo"}
                                </button>
                                <button
                                    onClick={onBack}
